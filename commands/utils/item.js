...
module.exports = class Item extends cmd.Command {
  ...
  run(message, { name }) {
    const items = itemList.root["道具"].filter(
      (item) =>
        item["基本名稱"] &&
        item["編號"] &&
        item["基本名稱"].indexOf(name) !== -1
    );
    ...
    const embeds = items.slice(0, 100).map((item) => {
      return new Discord.MessageEmbed()
        .setTitle(item["基本名稱"])
        .setURL(
          "https://ookamiwatari.github.io/le-ciel-bleu-db/#/item/" +
            item["編號"]
        )
...
function getDropList(item) {
  ...
  if (
    drops.filter((drop) => drop["怪物名稱"].startsWith("強欲蔵")).length > 1
  ) {
    isGreedyStorehouse = true;
    drops = drops.filter((drop) => !drop["怪物名稱"].startsWith("強欲蔵"));
  }
  if (
    drops.filter((drop) => drop["怪物名稱"].indexOf("幻想袋") !== -1).length > 1
  ) {
    isFantasyBag = true;
  ...
  return drops
    .map((drop) => {
      if (+drop["編號"] > 4214) {
        ...
        const monster = monsterList.root.npc.find(
          (monster) => monster["編號"] === drop["編號"]
        );
