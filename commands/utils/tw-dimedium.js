...
const stageList = require('../../json/tw/stage.json');
const questList = require('../../json/tw/quest.json');
const questStepinfoList = require('../../json/tw/quest_stepinfo.json');
const questSteplistList = require('../../json/tw/quest_steplist.json');

module.exports = class Dimedium extends cmd.Command {
	constructor(client) {
		super(client, {
			name: 'twdimedium',
			aliases: ['twdime', 'twdimesium', 'twdimegium', 'twdimejium', 'twmap', 'twstage', 'twjump', 'twgoto'],
			group: 'utils',
			memberName: 'twdimedium',
			description: "ディメジウム検索",
			examples: ['/twdimedium ドラグーンフォール C'],
			args: [
				{
					key: 'name',
	...
	run(message, {name, suffix}) {
		...
		const stages = stageList.root['場景'].filter((stage) => {
			...
			const stage_name = stage['名稱'].replace(/[Ａ-Ｚａ-ｚ０-９]/g, (s) => String.fromCharCode(s.charCodeAt(0) - 0xFEE0)).toUpperCase();
...
function getStageDescription (stage) {
	const infolists = questStepinfoList.root.info.filter((info) => info['地圖編號'] === stage['編號']);
	...
	return quests.slice(0, 30).map((quest) => {
		...
		str += quest['任務名稱']
		str += (quest['承接00'] && quest['地點00']) ? (': ' + quest['承接00'] + '(' + quest['地點00'] + ')') : '';
		return str;
	}).join('\n');
}
